...
parva_df = pd.read_csv('virata_parva.csv')
tippani_df = pd.read_csv('output.csv')

# Ensure padya columns are strings and handle NaN values
parva_df['padya'] = parva_df['padya'].astype(str).fillna('')
tippani_df['Padya'] = tippani_df['Padya'].astype(str).fillna('')
...
for tippani_index, tippani_row in tippani_df.iterrows():
    for parva_index, parva_row in parva_df.iterrows():
        if tippani_row['Sandhi'] == parva_row['sandhi']:
            if check_padya_ending(parva_row['padya'], tippani_row['Padya']):
                parva_df.at[parva_index, 'tippani'] = tippani_row['Tippani']

parva_df = parva_df.replace("nan", "-")

# Save the updated DataFrame back to Parva.csv
parva_df.to_csv('virata_parva_final.csv', index=False, encoding='utf8')
