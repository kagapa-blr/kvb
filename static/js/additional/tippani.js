...
$(document).ready(function () {
  ...
  const table = $("#dictionaryTable").DataTable({
    ...
    const preview = words.length > 5 ? words.slice(0, 5).join(" ") + " ..." : data;
    
    return `<span class="fw-bold" title="${escapeHtml(data)}">${escapeHtml(preview)}</span>`; },
      },
      {
        data: "parva_number",
        title: "ಪರ್ವ ಸಂಖ್ಯೆ",
  ...
  function applyClientFilters() {
    ...
    if (selectedLetter) {
      filteredRows = filteredRows.filter(
        (item) =>
          item.tippani &&
          item.tippani.trim().startsWith(selectedLetter),
      );
    }
  ...
  async function loadAkaradiSuchi(search = "") {
    ...
    try {
      ...
      const response = await apiClient.get(
        ApiEndpoints.ADDITIONAL_API.tippaniApi,
        { params: paramsObj }
      );
...
${highlightWord(data.padya || "", rowData.tippani || "")}
              </pre>
            </p>
